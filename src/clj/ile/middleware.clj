...
(defn wrap-security-header [handler]
  (fn [request]
    (if (:dev env)
      ...
      (->
        (handler request)
        (response/header "Content-Security-Policy" "default-src http:'" #_"default-src https:; img-src 'self'; script-src 'self'; style-src 'self'; font-src 'self'; object-src 'none'")
